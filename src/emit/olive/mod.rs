...
impl OliveTarget {
    ...
    fn declare_externals(&self, binary: &Binary) {
        ...
        binary.module.add_function(
            "popeye_make_message",
            ctx.i8_type().ptr_type(AddressSpace::Generic).fn_type(&[], false),
        Some(Linkage::External),
        );
        binary.module.add_function(
            "popeye_make_message_length",
            ctx.i8_type().ptr_type(AddressSpace::Generic).fn_type(&[], false),
            Some(Linkage::External),
        );
        binary.module.add_function(
            "popeye_make_object",
            ctx.i8_type().ptr_type(AddressSpace::Generic).fn_type(&[u64_val], false),
            Some(Linkage::External),
        );
        binary.module.add_function(
            "popeye_make_named_object",
            ctx.i8_type().ptr_type(AddressSpace::Generic).fn_type(&[u64_val, u8_ptr], false),
            Some(Linkage::External),
        );
    ...
    // Emit popeye_main function for analysis
    // fn emit_popeye_main(&mut self, binary: &mut Binary, contract: &ast::Contract, ns: &ast::Namespace) {
    //
    // }
    //
    // fn emit_global_variables(&mut self, binary: &mut Binary, contract: &ast::Contract, ns: &ast::Namespace) {
    //     for layout in contract.layout {
    //         let var = &contract.variables[layout.var_no];
    //
    //         if let Some(init) = &var.initializer {
    //             let storage = contract.get_storage_slot(
    //                 pt::Loc::Codegen,
    //                 layout.contract_no,
    //                 layout.var_no,
    //                 ns,
    ...
    pub fn encoded_length<'x>(
        ...
        match ty {
            ...
            ast::Type::Enum(n) => OliveTarget::encoded_length(
                arg,
                load,
                packed,
                &ns.enums[*n].ty,
            ...
            ast::Type::Ref(r) => {
                OliveTarget::encoded_length(arg, load, packed, r, function, binary, ns)
            }
            ast::Type::String | ast::Type::DynamicBytes => {
                let arg = if load {
                    binary.builder.build_load(arg.into_pointer_value(), "")
