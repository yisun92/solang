...
use crate::emit::olive::{event_id, SCRATCH_SIZE, OliveTarget};
use crate::emit::{TargetRuntime, Variable};
use crate::sema::ast;
use crate::sema::ast::{Function, Namespace, Type};
use crate::{codegen, emit_context};
use inkwell::types::{BasicType, IntType};
use inkwell::values::{ArrayValue, BasicMetadataValueEnum, BasicValueEnum, CallableValue, FunctionValue, IntValue, PointerValue, BasicValue};
use inkwell::{AddressSpace, IntPredicate};
use inkwell::values::AnyValue;
use solang_parser::pt;
use std::collections::HashMap;

impl<'a> TargetRuntime<'a> for OliveTarget {
    ...
    /// Modified to ignore failed seal_get_storage
    fn get_storage_int(
        &self,
        binary: &Binary<'a>,
        ...
        loaded_int.into_int_value()
    }


    /// Read string from substrate storage
    /// Modified to remove all checks
    ...
    fn set_storage_bytes_subscript(
        ...
        let length = binary.builder.build_load(scratch_len, "string_len").into_int_value();
    ...
    fn abi_encode_to_vector<'b>(
        ...
        for arg in packed {
            ...
            length = binary.builder.build_int_add(
                length,
                OliveTarget::encoded_length(*arg, false, true, ty, function, binary, ns),
                "",
            );
        }
        ...
        for arg in args {
            ...
            length = binary.builder.build_int_add(
                length,
                OliveTarget::encoded_length(*arg, false, false, ty, function, binary, ns),
                "",
            );
        }
    ...
    fn abi_encode<'b>(
        ...
        for (i, ty) in tys.iter().enumerate() {
            length = binary.builder.build_int_add(
                length,
                OliveTarget::encoded_length(args[i], load, false, ty, function, binary, ns),
                "",
            );
        }
